    ...
    void MovePuzzle(){
        foreach (PuzzleTile tile in puzzleTiles)
        {
            ...
            if(tile.clicked){
                ...
                float x = (tile.transform.position.x + colliderCenter.x - colliderSize.x/2) + colliderSize.x /2;
                ...
                float y = (tile.transform.position.y + colliderCenter.y - colliderSize.y/2) + colliderSize.y /2;
                float xRight = tile.transform.position.x + colliderCenter.x + colliderSize.x/2 * direction;
                float xLeft = tile.transform.position.x + colliderCenter.x + colliderSize.x/2 * -direction;
                ...
                if((Physics.Raycast(rayUp, out hit, 1.0f, collisionMask) == false) && (tile.moved == false) && (tile.transform.position.y < startPosition.y)){
                    tile.moveUp = true;
                }
                if(Physics.Raycast(rayDown, out hit, 1.0f, collisionMask) == false && !tile.moved && tile.transform.position.y > (startPosition.y - 2 * offset.y)){
                    tile.moveDown = true;
                }

                if(!Physics.Raycast(rayRight, out hit, 1.0f, collisionMask) && !tile.moved && tile.transform.position.x < (startPosition.x + 2 * offset.x)){
                    tile.moveRight = true;
                }
                if(!Physics.Raycast(rayLeft, out hit, 1.0f, collisionMask) && !tile.moved && tile.transform.position.x > startPosition.x){
                    tile.moveLeft = true;
                }
            }
        }
